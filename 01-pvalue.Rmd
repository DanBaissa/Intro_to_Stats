...
data_for_plot <- data.frame(z_scores, density_values) %>%
  mutate(
    fill_color = case_when(
      (z_scores < -2.58 | z_scores > 2.58) ~ "blue",
...
p_value_plot <- ggplot(data_for_plot, aes(x = z_scores, y = density_values)) +
  ...
  labs(
    title = "Visualizing two tail p-values in terms of Sigma",
    subtitle = "Red: p < 0.05, Blue: p < 0.01",
    x = "Z-score (Sigma)",
    y = "Density",
    fill = "Fill Color" 
  ) +
...
- **Red Zones:** The red vertical lines show where our results would fall if they were more than 1.96 standard deviations away from the mean (either side). Statistically, this represents a p-value less than 0.05, where many scientific fields consider a finding statically significant.
